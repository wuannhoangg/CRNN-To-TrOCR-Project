# src/crnn/predict.py
"""
Chạy suy luận (predict) cho mô hình CRNN OCR.

Hỗ trợ:
- Nhận dạng 1 ảnh qua --image
- Nhận dạng hàng loạt trong thư mục qua --images_dir
"""
...
def preprocess_image(path, height, pad_w):
    """Tiền xử lý ảnh để phù hợp với đầu vào của CRNN.

    Args:
        path (str): Đường dẫn ảnh đầu vào.
        height (int): Chiều cao chuẩn hoá (giống lúc huấn luyện).
        pad_w (int): Chiều rộng cố định/padding (giống lúc huấn luyện).

    Returns:
        torch.Tensor: Tensor ảnh dạng (1, H, W), chuẩn hoá về [0, 1].

    Raises:
        FileNotFoundError: Nếu không đọc được ảnh từ đường dẫn.
    """
...
def main():
    """Parse tham số CLI, nạp checkpoint và chạy suy luận."""
    ...
    ap.add_argument('--images_dir', type=str, help='Thư mục ảnh để nhận dạng hàng loạt')
